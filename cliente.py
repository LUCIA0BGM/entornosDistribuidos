...
def obtener_estado_pedidos():
    try:
        cliente_socket.sendall(b"estado_pedido")
        return cliente_socket.recv(1024).decode()
    except Exception as e:
        print(f"❌ Error al obtener estado de pedidos: {e}")
        return None

def recoger_pedido():
    try:
        cliente_socket.sendall(b"recoger_pedido")
        return cliente_socket.recv(1024).decode()
    except Exception as e:
        print(f"❌ Error al recoger pedido: {e}")
        return None

def entregar_pedido(mesa):
    try:
        mensaje = f"entregar_pedido {mesa}".encode()
...
def move_robot():
    ...
    while running:
        if len(delivering_orders) == 0 and len(prepared_orders) > 0:
            ...
            while len(delivering_orders) < CAPACIDAD_MAXIMA:
                respuesta = recoger_pedido()
                if "Pedido" in respuesta:
                    mesa = int(respuesta.split()[1])
                    delivering_orders.append(mesa)
                    print(f"📦 Robot recoge pedido para mesa {mesa}")
                else:
                    print(f"⚠️ {respuesta}")
                    break
            ...
            for mesa_numero in ruta_optima:
                ...
                if destino:
                    ...
                    respuesta = entregar_pedido(mesa_numero)
                    print(f"📤 {respuesta}")
